({
	setColumns : function(Component,event) {
     ...
     getOwnershipVals : function(component, event) {
		
        var storePickVals = component.get("c.getProduct");
       
        storePickVals.setCallback(this, function(response){
            var state = response.getState();
            if (state === "SUCCESS") {
                var  returnvalue = response.getReturnValue();
